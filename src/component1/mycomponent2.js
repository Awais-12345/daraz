import React from "react";
import footer1 from "../component2/mapping1/map5p1";
import footer2 from "../map5p2";
import footer3 from "../map5p3";
const MyComponent2 = () => {
  return (
    <div>
      <div className="content5">
        <div className="first-step">
          <div className="heading1">
            <div>
              <h3>Payment Methods</h3>

              <div className="allimg1">
                {footer1.map(
                  (All) => (
                    <img src={All.image1} key={All.id} />
                  )
                  // {/* <img
                  //   loading="lazy"
                  //   src="https://img.lazcdn.com/us/domino/78355368-8518-4b88-9f8e-e5751ee0863a_PK-140-84.png"
                  //   alt="visa"
                  // />
                  // <img
                  //   loading="lazy"
                  //   src="https://img.lazcdn.com/us/domino/5ba8a652-1445-45cb-a2a5-ed6043004243_PK-63-48.png"
                  //   alt="ms"
                  // />
                  // <img
                  //   loading="lazy"
                  //   src="https://img.lazcdn.com/us/domino/dd8e0447-8052-44f3-8ab2-b24c56a2491f_PK-139-84.png"
                  //   alt="easy-pay"
                  // />
                  // <img
                  //   loading="lazy"
                  //   src="https://img.lazcdn.com/us/domino/e5c3cd93-b796-410f-9bfd-fbfdaa055149_PK-144-84.png"
                  //   alt="daraz-wallet"
                  // />
                  // <img
                  //   loading="lazy"
                  //   src="https://img.lazcdn.com/us/domino/56915ba8-b2c0-4caf-b3a1-7e0f3d2d45cf_PK-139-84.png"
                  //   alt="jazz-cash"
                  // />
                  // <img
                  //   loading="lazy"
                  //   src="https://img.lazcdn.com/us/domino/75489476-3c86-44ed-bf81-8b0579d56e1c_PK-139-84.png"
                  //   alt="union-pay-mm"
                  // />
                  // <img
                  //   loading="lazy"
                  //   src="https://img.lazcdn.com/us/domino/6668ff96-bf9a-40bd-8ce4-f7f7c0bee385_PK-144-84.png"
                  //   alt="hbl"
                  // />
                  // <img
                  //   loading="lazy"
                  //   src="https://img.lazcdn.com/us/domino/7237a412-04cc-4571-a200-44f6f847a9da_PK-144-84.png"
                  //   alt="easy-monthly-installments"
                  // /> */}
                )}
              </div>
            </div>
            <div className="heading2">
              <h3>Verified by</h3>
              <div className="res">
                <img src="//img.drz.lazcdn.com/g/tps/imgextra/i4/O1CN01ZaMORP1I3qlBom0V2_!!6000000000838-2-tps-73-41.png" />
              </div>
            </div>
          </div>
          <div className="second-step">
            <div className="first-portion">
              <h3>Daraz International</h3>
              <div className="allpic1">
                {footer2.map((ele)=>
                <>
                <img src={ele.image3} key={ele.id} />
                <span>{ele.country}</span>
              
                
                {/* <img
                //    loading="lazy"
                //    src="https://img.lazcdn.com/us/domino/05a4ef2c-095c-407d-9295-aa45b5412de0_PK-84-84.png"
                //      alt="bd"
                //   />
                //   <span>Bangladesh</span> 
                //   <img
                //      loading="lazy"
                //     src="https://img.lazcdn.com/us/domino/5536488d-c32a-44eb-8307-ed6651d907da_PK-84-84.png"
                //      alt="lk"
                //    /> 
                //   <span>Sri Lanka</span> 

                //   <img
                //     loading="lazy"
                //      src="https://img.lazcdn.com/us/domino/8896f696-b36f-4cb1-8576-2e86eed05046_PK-84-84.png"
                //      alt="mm"
                //    /> 
                //   <span>Myanmar</span> 
                //  <img
                //      loading="lazy"
                //      src="https://img.lazcdn.com/us/domino/39f7f111-619f-47cd-993c-a551aaed15fd_PK-86-86.png"
                //      alt="np"
                //    /> 
                //  <span>Nepal</span>  */}
                </>
              )}
              </div>
            </div>
            <div className="second-portion">
              <h3>Follow Us</h3>
              <div className="allpic2">
                {footer3.map((ele) => (
                  <img src={ele.image4} key={ele.id}/>
                  //   {/* <img
                  //   loading="lazy"
                  //   src="https://img.lazcdn.com/us/domino/77ab92ab-17ae-40ce-aa8b-e8de5974939d_PK-76-76.png"
                  //   alt="twitter"
                  // />
                  // <img
                  //   loading="lazy"
                  //   src="https://img.lazcdn.com/us/domino/f03a43e7-3655-4049-8c12-b1614ac2a2d4_PK-75-76.png"
                  //   alt="instagram"
                  // />
                  // <img
                  //   loading="lazy"
                  //   src="https://img.lazcdn.com/us/domino/6bf9555a-40ae-466d-a756-907f70aa3461_PK-76-76.png"
                  //   alt="youtube"
                  // />
                  // <img
                  //   loading="lazy"
                  //   src="https://img.lazcdn.com/us/domino/cea1aedb-aa69-44d5-bbaf-ca5901797dea_PK-76-76.png"
                  //   alt="darazBlog"
                  // /> */}
                ))}
              </div>
            </div>
            <div className="third-portion">© Daraz 2024</div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default MyComponent2;
